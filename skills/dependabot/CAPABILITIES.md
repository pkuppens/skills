...
| `allowed-tools` | **Used, narrowly** | The one Claude Code-only field the spec *does* allow (marked experimental). Pre-approves only the **read-only** `gh` calls (`pr list`, `pr view`, `pr diff`, `pr checks`, `run list`, `run view`, `repo view`). Deliberately excludes `gh pr review` / `gh pr comment` / `gh pr merge` — those stay behind a normal permission prompt because they're the write, visible-to-others steps. |
